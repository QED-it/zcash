...
#include "asyncrpcoperation_sendassets.h"
...
AsyncRPCOperation_sendassets::AsyncRPCOperation_sendassets(
        ...
        mindepth_(minDepth), anchordepth_(anchorDepth), strategy_(TransactionStrategy(PrivacyPolicy::FullPrivacy)),
        contextinfo_(contextInfo), fee_(std::nullopt)
{
    assert(mindepth_ >= 0);
    assert(!recipients_.empty());
    assert(ztxoSelector.RequireSpendingKeys());

    // Log the context info i.e. the call parameters to z_sendassets
    if (LogAcceptCategory("zrpcunsafe")) {
        LogPrint("zrpcunsafe", "%s: z_sendassets initialized (params=%s)\n", getId(), contextInfo.write());
    } else {
        LogPrint("zrpc", "%s: z_sendassets initialized\n", getId());
    }
}
...
void AsyncRPCOperation_sendassets::main() {
    if (isCancelled())
        return;
    ...
    std::string s = strprintf("%s: z_sendassets finished (status=%s", getId(), getStateAsString());
    if (txid.has_value()) {
        s += strprintf(", txid=%s)\n", txid.value().ToString());
    } else {
        s += strprintf(", error=%s)\n", getErrorMessage());
...
AsyncRPCOperation_sendassets::main_impl(CWallet& wallet) {

    auto asset = recipients_[0].GetAsset(); // TODO support multiple assets

    auto spendable = builder_.FindAllSpendableAssets(wallet, asset, ztxoSelector_, mindepth_);
 ...
 */
UniValue AsyncRPCOperation_sendassets::getStatus() const {
    ...
    obj.pushKV("method", "z_sendassets");
